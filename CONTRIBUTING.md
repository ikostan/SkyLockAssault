# Contributing to Sky Lock Assault

Thank you for your interest.

**This project is currently closed to external contributions.**

The source is available under the PolyForm Noncommercial License 1.0.0 for non-commercial
use only. We are not accepting pull requests, code contributions, or documentation
changes from external contributors at this time.

If you find a bug or have feedback, you are welcome to open a GitHub issue. Feature
requests and discussions are also fine.

For commercial licensing or other inquiries, please open a GitHub issue or contact
via X @EgorKostan.

Thank you for understanding.
