...
## Ensure initial focus is set for the menu if not already present.
##
## Checks whether keyboard or controller focus is already inside this menu. If it isn't, defers
## grab_focus() on the candidate and logs the action. If it is, logs the skip so you can still
## see what happened.
##
...
##
## Loads the Key Mapping menu directly while keeping background video visible. Uses robust
## video lookup for both Panel and root Control.
##
...
## Log a message to the console with a timestamp and log level.
##
## Prints a formatted log message if the specified level meets or exceeds the current settings
## threshold.
##
...
## Returns the version string configured in ProjectSettings.
##
## Retrieves the application version from the project settings. Defaults to "n/a" if not found.
...
## Sets the game version for test execution.
##
## Modifies the project settings application/config/version directly to the supplied value.
##
...
##
## Checks the magic number of the file at the given path to determine if it is encrypted using
## Godot's encryption format.
##
...
##
## Loads the configuration from the specified file path, automatically detecting if the file is
## encrypted or uses legacy plaintext formatting.
##
## [param path]: The path parameter.
## Returns A Dictionary containing 'config' as a ConfigFile, 'err' as an int, and 'is_legacy'
## as a bool.
