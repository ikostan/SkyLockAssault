...
## Centralized helper that ensures initial keyboard or controller focus.
##
## Checks whether focus is already inside this menu. If it isn't, defers grab_focus() on the
...
## Load the key mapping menu while keeping the background video visible.
##
...
##
## Converts the log level to a string, prepends a system timestamp, and outputs the result
## unless filtered out by settings.
##
...
## Returns the current game version from project settings.
## Returns The game version string, or "n/a" if not set.
...
## Sets the game version for tests in the project settings.
##
...
##
## Checks the magic number of the file at the specified path to verify if it is an encrypted
## Godot file.
##
## [param path]: The path parameter.
## Returns Returns [code]true[/code] if the file is encrypted with the GDEC magic number,
...
##
## Loads a configuration file from disk, automatically managing encryption keys and falling
## back to legacy plaintext if necessary.
##
## [param path]: The path parameter.
## Returns A Dictionary containing the loaded [code]ConfigFile[/code] under "config", the error
## code under "err", and a [code]bool[/code] indicating legacy status under "is_legacy".
