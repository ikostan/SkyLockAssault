...
## Ensure initial focus is set within the menu context.
##
## Checks whether keyboard or controller focus is already inside this menu. If it is not,
## defers grab_focus() on the candidate and logs the action. If it is, logs the skip.
##
...
##
## Loads the key mapping scene while hiding the specified menu and ensuring the background
## video player remains visible and processing.
##
...
## Logs a formatted message to the console with a specific log level and timestamp.
##
## Checks against the configured log level threshold before printing the message.
##
...
##
## Updates the application configuration version setting in the project settings to the
## specified value.
##
...
##
## Checks the magic number of the file at the given path to see if it matches the Godot
## Encrypted File Magic Number (0x43454447 or "GDEC"). Returns false if the file does not
## exist, cannot be opened, or is too short.
##
...
##
## Handles both encrypted and legacy plaintext formats. Automatically checks file existence and
## encryption status, using the encryption key when necessary.
##
## [param path]: The path parameter.
## Returns Returns a Dictionary containing config, err, and is_legacy.
