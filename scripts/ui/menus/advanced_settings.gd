...
func _on_fps_toggle_toggled(toggled_on: bool) -> void:
	...
	if os_wrapper.has_feature("web") and js_bridge_wrapper:
		...
		js_bridge_wrapper.eval(
			(
				"var fpsEl = document.getElementById('fps-toggle'); if (fpsEl) fpsEl.checked = "
				+ js_bool
				+ ";"
			),
			true
...
func _on_fps_toggle_js(args: Array) -> void:
	...
	if first_arg is JavaScriptObject or typeof(first_arg) == TYPE_ARRAY:
		...
		if (
			...
			or (typeof(first_arg) == TYPE_ARRAY and first_arg.is_empty())
		):
			Globals.log_message(
				"JS toggleFps callback received an empty array—skipping.", Globals.LogLevel.WARNING
			)
			return
