...
func _on_resume_button_pressed() -> void:
	Globals.log_message("Resume button pressed.")
	toggle_pause()
...
func _on_back_to_main_button_pressed() -> void:
	Globals.log_message("Back To Main Menu button pressed.")
	get_tree().paused = false  # Always unpause before scene change
	visible = false  # Extra: Force hide for smooth transition
	# Runtime load—avoids export null/cycle
	var main_menu_scene: PackedScene = load("res://scenes/main_menu.tscn")
	if main_menu_scene:
		Globals.log_message("Switch back to Main menu scene.")
		get_tree().change_scene_to_packed(main_menu_scene)
	else:
		Globals.log_message("Error: Failed to load main_menu.tscn—check path.")
