...
class DummyTracing:
    ...
    def stop(self, *, path: str | Path | None = None, **_: Any) -> None:
        if path is not None:
...
class DummyPage:
    ...
    def screenshot(self, path: str | Path, **_: Any) -> None:
...
def isolate_conftest_state(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> Generator[Path, None, None]:
    ...
    conftest._FAILED_NODEIDS.clear()


def test_failing_test_retains_trace_screenshot_and_video(isolate_conftest_state: Path) -> None:
    """A failing test should retain trace, screenshot, and video artifacts."""
    artifacts_dir = isolate_conftest_state
    context = DummyContext()
    page_obj = DummyPage(artifacts_dir / "temp_video.webm")
    request = _make_request(call_failed=True)

    _cleanup_context_diagnostics(context, page_obj, request, include_module_failures=False)
...
def test_passing_test_purges_failure_artifacts(isolate_conftest_state: Path) -> None:
    ...
    page_obj = DummyPage(video_file)
    request = _make_request(call_failed=False)

    _cleanup_context_diagnostics(context, page_obj, request, include_module_failures=False)

    assert not list(artifacts_dir.glob("trace_*.zip")), "Trace should not be saved on pass"
    assert not list(artifacts_dir.glob("failure_*.png")), "Screenshot should not be created on pass"
    assert not video_file.exists(), "Video should be deleted on pass"
    assert context.closed, "Context should be closed"


def test_module_level_failure_preserves_module_scoped_diagnostics(isolate_conftest_state: Path) -> None:
    ...
    _cleanup_context_diagnostics(context, page_obj, request, include_module_failures=True)

    assert list(artifacts_dir.glob("trace_*.zip")), "Trace should be retained for module setup failure"
    assert list(artifacts_dir.glob("video_*.webm")), "Video should be retained for module setup failure"
    assert context.closed, "Context should be closed"
