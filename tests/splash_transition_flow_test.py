...
# ---------------------------------------------------------------------------
# Unit tests for the `onProgress` telemetry callback introduced in
# custom_shell.html. These tests extract the real callback source verbatim
# (via brace-matching, not a hardcoded copy) and exercise it in an isolated
# JS context, independent of the Godot WASM boot flow / dev server.
# ---------------------------------------------------------------------------
...
def _read_custom_shell_source() -> str:
    """Reads the raw custom_shell.html source from the repository root."""
    return _CUSTOM_SHELL_PATH.read_text(encoding="utf-8")


def _extract_on_progress_function_source() -> str:
    """Extracts the `onProgress` telemetry callback verbatim from custom_shell.html.

    Uses brace counting (rather than a fixed-indentation regex) so extraction
    keeps working even if the surrounding code is reformatted or re-indented.
    """
    html = _read_custom_shell_source()
    ...
    while idx < len(html):
        ...
        raise AssertionError(
            "Could not find matching closing brace for onProgress function body"
        )
...
def test_on_progress_telemetry_math_is_correct(page: Page) -> None:
    """
    Unit-tests the `onProgress` telemetry callback added to custom_shell.html
    in isolation, locking down its percentage math independently of the full
    Godot WASM boot flow exercised by `test_splash_transition_flow`.
    """
...
def test_on_progress_floors_fractional_percentages(page: Page) -> None:
    ...
    outputs = _run_on_progress(
        ...
        fn_source,
        [
            (1, 3),  # 33.33...% -> must floor to 33%, not round to 33% or 34%
            (2, 3),  # 66.66...% -> must floor to 66%, not round to 67%
            (99, 100),  # 99% exactly
        ],
    )
...
def test_on_progress_skips_logging_when_total_is_zero(page: Page) -> None:
    """
    Edge case: onProgress must not emit any telemetry (or divide by zero,
    producing NaN/Infinity) when `total` is 0. This guards the
    `if (total > 0)` check in the callback.
    """
    fn_source = _extract_on_progress_function_source()

    outputs = _run_on_progress(page, fn_source, [(0, 0), (5, 0)])

    assert outputs == [], f"Expected no telemetry logs when total is 0, got: {outputs}"


def test_on_progress_message_format_matches_expected_pattern(page: Page) -> None:
    """
    Regression test: the emitted telemetry text must match the exact
    "Telemetry - Assembly Transfer: <int>%" format that
    `test_splash_transition_flow` parses with a regular expression.
    """
    fn_source = _extract_on_progress_function_source()

    outputs = _run_on_progress(page, fn_source, [(37, 41)])

    assert len(outputs) == 1
    assert re.fullmatch(
        r"Telemetry - Assembly Transfer: \d+%", outputs[0]
    ), f"Unexpected telemetry message format: {outputs[0]!r}"


# ---------------------------------------------------------------------------
# Additional integration-level checks against the live Godot HTML5 export,
# complementing the monotonicity/bounds assertions already covered by
# `test_splash_transition_flow`.
# ---------------------------------------------------------------------------


def test_splash_transition_flow_progress_reaches_completion(page: Page) -> None:
    """
    Regression test: telemetry must not merely progress monotonically, it must
    actually approach completion (>=90%) by the time the engine reports
    itself initialized, guarding against a stalled/truncated progress stream.
    """
    logs: list[dict[str, str]] = []
    ...
    try:
        ...
        assert progress_values, "No telemetry captured during engine boot."
        assert max(progress_values) >= 90, (
            "Assembly transfer telemetry never approached completion before "
            f"godotInitialized became true: {progress_values}"
        )
    finally:
        try:
            page.remove_listener("console", on_console)
...
def test_splash_transition_flow_no_malformed_telemetry_values(page: Page) -> None:
    """
    Negative test: guards against malformed telemetry (e.g. "NaN%" or
    "Infinity%") that would appear if the `total > 0` division-by-zero guard
    in the onProgress callback were ever removed or broken.
    """
    logs: list[dict[str, str]] = []
    ...
    try:
        ...
        assert malformed == [], f"Malformed telemetry entries found: {malformed}"
    finally:
        try:
            page.remove_listener("console", on_console)
        except Exception:
