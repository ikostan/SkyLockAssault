...
| Strengthened the salt injection script to be strictly fail-fast, newline-normalizing, and self-verifying while handling unset/empty secrets safely.    | <ul><li>Enable strict bash options (set -euo pipefail) and tolerate missing positional arg using default expansion for the target file.</li><li>Guard against missing target file and unset PRODUCTION_SALT using parameter expansion, with clear error messaging instead of unbound-variable crashes.</li><li>Normalize PRODUCTION_SALT by stripping all CR/LF characters into CLEAN_SALT, then escape it separately for Godot and sed.</li><li>Add an explicit empty-secret guard after normalization so secrets that become empty after stripping are rejected.</li><li>Use shared ci_utils.sh sed wrapper to perform the placeholder replacement and add a post-injection grep check that fails if the placeholder remains, printing success otherwise.</li></ul>                                                                                                                                                                                                                                                                                                                    | `.github/scripts/inject_salt.sh`                                                                                                                                                                                       |
| Made the itch.io deployment workflow reliably export, patch, re-zip, verify, and deploy the patched Web build with stronger step-level error handling. | <ul><li>Add set -euo pipefail to critical workflow run steps, including version injection, plugin disabling, salt injection, export staging, index.js patching, re-zipping, and butler setup/deploy.</li><li>Create and clean a dedicated patched_build subdirectory under the export archive_directory before unzipping the Godot Web.zip into it.</li><li>Run patch_index_js.sh on the patched_build directory instead of the previous static export/web path to keep all mutations scoped to the exported artifacts.</li><li>Delete the original unpatched Web.zip, then zip the contents of patched_build back into a Web.zip located at the same archive_directory path via pushd/popd and an absolute output path.</li><li>Introduce a verification step that streams the final Web.zip through unzip -p and uses grep to ensure the CI_INJECT_SALT_HERE placeholder does not appear in the shipped archive, failing otherwise.</li><li>Ensure the butler push step deploys the verified Web.zip from steps.export.outputs.archive_directory with strict error handling.</li></ul> | `.github/workflows/deploy_to_itch.yml`                                                                                                                                                                                 |
| Expanded and refined CI tests for the injection script to cover unset, empty-after-stripping, and complex multiline/whitespace secrets.                | <ul><li>Update run_injection helper to optionally simulate an entirely unset PRODUCTION_SALT by removing it from the environment when raw_secret is None, while preserving WSLENV handling.</li><li>Parametrize the empty-secret test to cover empty strings and secrets that are only newlines, asserting non-zero exit and untouched target files.</li><li>Replace the previous failing-multiline-secret test with a new parametrized test that ensures multiline and CRLF secrets are newline-stripped and still injected correctly, with exact expected injected values including whitespace-only cases.</li><li>Add a dedicated test for an unset PRODUCTION_SALT environment variable that expects graceful failure with a custom error message, no bash 'unbound variable' errors, and no file modifications.</li></ul>                                                                                                                                                                                                                                                           | `tests/ci/test_salt_injection.py`                                                                                                                                                                                      |
| Improved runtime observability of encryption key generation and updated documentation to reflect the hardened pipeline.                                | <ul><li>In the encryption key generation function, compute the final key into a local variable and log its length at DEBUG level before returning, to aid debugging of salt injection issues.</li><li>Extend milestone documentation to explicitly mention the hardened CI salt injection handling of unset/multiline PRODUCTION_SALT and the adjusted itch.io workflow behavior around patched exports.</li></ul>                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | `scripts/core/globals.gd`<br/>`files/docs/milestones/18/PART_1_Encryption_key_generation_failure_in_web_export.md`                                                                                                     |
