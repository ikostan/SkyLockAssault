...
| Expose key Godot settings state to the browser and optimize high-frequency setting change handling for CI tests.                                                                                   | <ul><li>In _ready(), on web builds set window.godotInitialized and initialize window.currentLogLevel via JavaScriptBridge.</li><li>Refine _on_setting_changed to early-return during bulk settings loading, avoiding persistence, logging, and JS bridge churn.</li><li>Special-case current_fuel to push live values to window.currentFuel, only log when log level is DEBUG, and skip disk saves to avoid I/O/log spam.</li><li>Mirror current_log_level updates into window.currentLogLevel when running on web builds.</li><li>Always log non-fuel setting changes at DEBUG and persist them via _save_settings, and adjust placeholder encryption salt string for CI.</li><li>(Implied elsewhere) Ensure UTF-8 encoding is used where these settings are persisted or logged for deterministic CI behavior.</li></ul>                                                                                                                                                   | `scripts/core/globals.gd`                                                                                                                                                                                                                                                                                                                                                                                                                                           |
