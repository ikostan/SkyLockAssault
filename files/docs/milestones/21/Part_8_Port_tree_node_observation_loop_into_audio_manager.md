...
|-----------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------|
| Centralize global Button pressed SFX wiring and scene-tree observation in AudioManager, including retroactive scanning and strict listener registration.  | <ul><li>Connect AudioManager._on_node_added to SceneTree.node_added in _ready() with a guard to ensure a single listener.</li><li>Introduce _retroactive_ui_scan to recursively traverse the scene tree and feed existing nodes into the same button-parsing logic.</li><li>Implement _on_node_added in AudioManager to filter flat buttons, metadata-based opt-outs, and AcceptDialog internals before wiring pressed signals with CONNECT_DEFERRED.</li><li>Implement _on_global_button_pressed in AudioManager to play the ui_accept sound via the configured SFX menu bus.</li><li>Remove the Globals-based node_added listener and its button hook/audio callback implementations to fully migrate responsibility into AudioManager.</li></ul>                                                                                            | `scripts/managers/audio_manager.gd`<br/>`scripts/core/globals.gd`                           |
| Update and extend GUT tests to target AudioManager’s button hook behavior, retroactive scanning, idempotency, and listener singularity.                   | <ul><li>Switch test helpers to look for AudioManager._on_global_button_pressed instead of Globals._on_global_button_pressed when counting connections.</li><li>Update duplicate-scan test to call AudioManager._on_node_added and assert that connections remain single.</li><li>Add tests covering retroactive scan wiring of pre-existing buttons, idempotent behavior on already-wired hierarchies, and strict singular registration of the node_added listener.</li><li>Add deep-hierarchy retroactive scan tests ensuring nested buttons are wired while container nodes remain untouched.</li><li>Refactor quit dialog SFX suite to use an inherited AudioManager script double that overrides play_sfx for spying while preserving real wiring behavior, and drop the obsolete flat-button anti-trigger test tied to Globals.</li></ul> | `test/gut/test_globals_button_hooks.gd`<br/>`test/gut/test_quit_game_confirm_dialog_sfx.gd` |
| Tighten GitHub Actions workflows for browser tests and GDScript linting/formatting to satisfy yamllint and modern tooling expectations.                   | <ul><li>Update browser_test.yml quoting, indentation, and if-condition expressions to be consistently double-quoted and lint-compliant.</li><li>Adjust Playwright cache key to depend on the installed Playwright version and add yamllint disable/enable comments for long lines.</li><li>Reformat the embedded Python HTTP server block for readability within lint constraints.</li><li>Switch gdlint workflow from the Scony/godot-gdscript-toolkit action to installing gdtoolkit via pip directly.</li></ul>                                                                                                                                                                                                                                                                                                                             | `.github/workflows/browser_test.yml`<br/>`.github/workflows/gdlint.yml`                     |
| Add milestone documentation describing the port of the tree node observation loop into AudioManager and summarizing the architectural/testing/CI changes. | <ul><li>Create a new markdown milestone file outlining the move of button SFX wiring from Globals to AudioManager.</li><li>Document key refactoring points, test updates, CI workflow adjustments, and the benefits of the change.</li><li>Include a concise reviewer’s guide and AI/bot contribution notes for the milestone.</li></ul>                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | `files/docs/milestones/21/Part_8_Port_tree_node_observation_loop_into_audio_manager.md`     |
