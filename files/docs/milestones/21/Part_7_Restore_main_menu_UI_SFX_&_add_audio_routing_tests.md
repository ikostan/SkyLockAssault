...
|------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Decouple UI navigation sound configuration from globals and move it into audio constants/resources.                                                  | <ul><li>Remove UI navigation sound path constant and preloaded stream from the global core node.</li><li>Delete the unused internal helper that played navigation SFX from globals.</li><li>Introduce the UI_NAV_SOUND_PATH constant in the centralized audio constants resource.</li></ul>                                                                                                                                                                                                        | `scripts/core/globals.gd`<br/>`scripts/resources/audio_constants.gd`                                                                                                          |
| Ensure main menu and global button interactions play confirmation/cancel SFX through the dedicated menu bus, even when global sounds are suppressed. | <ul><li>Route global button confirmation SFX through the BUS_SFX_MENU instead of the generic gameplay SFX bus.</li><li>Add explicit local ui_accept playback on the menu bus for Start, Options, and Quit main menu buttons that use no_global_sound metadata.</li><li>Update quit dialog confirm/cancel handlers to play ui_accept/ui_cancel via the menu bus and add missing logging for quit presses.</li></ul>                                                                                 | `scripts/core/globals.gd`<br/>`scripts/ui/menus/main_menu.gd`                                                                                                                 |
| Expose audio routing diagnostics and shared headless audio bus setup utilities for tests.                                                            | <ul><li>Add a public get_active_sfx_bus_name() API that returns the bus of the most recently started SFX.</li><li>Register GutTestHelper as a globally available class and add a bootstrap_headless_audio_buses() helper to DRY audio bus initialization.</li><li>Minor tidy-up of existing test helper code (e.g., collision node naming).</li></ul>                                                                                                                                              | `scripts/managers/audio_manager.gd`<br/>`test/gut/gut_test_helper.gd`                                                                                                         |
| Add integration and automation tests to verify button audio routing and main menu SFX behavior, with robust scene/audio cleanup.                     | <ul><li>Create test_main_menu_audio to load the main menu scene, simulate button presses, and assert ui_accept playback and asset path usage.</li><li>Create test_globals_button_routing to verify global button presses route to the menu bus, respect suppression for flat/metadata-muted buttons, and exclude dialog internal buttons.</li><li>Implement scene tree snapshotting and cleanup logic in tests to prevent leaked scenes, root children, and audio resources across runs.</li></ul> | `test/gut/test_main_menu_audio.gd`<br/>`test/gut/test_globals_button_routing.gd`<br/>`test/gut/test_globals_button_routing.gd.uid`<br/>`test/gut/test_main_menu_audio.gd.uid` |
| Polish comments and documentation around logging, encryption, and milestone tracking without changing runtime behavior.                              | <ul><li>Clarify and reflow comments in globals around log level guarding, encryption key generation, CI salt injection, and WebAssembly/JavaScriptBridge constraints.</li><li>Add a milestone documentation file describing the audio refactor, routing fixes, tests, and AI/bot contributions.</li></ul>                                                                                                                                                                                          | `scripts/core/globals.gd`<br/>`files/docs/milestones/21/Part_7_Restore_main_menu_UI_SFX_&_add_audio_routing_tests.md`                                                         |
