...
func before_each() -> void:
	# Ensure a fresh resource for every test to isolate state [cite: 58, 185]
	...
	# Inject mock wrapper to bypass real web/OS calls [cite: 194]
...
func test_gs_ui_01_03_slider_updates_resource_and_label() -> void:
	...
	# Simulate user sliding the control [cite: 208]
	gameplay_menu.difficulty_slider.value = test_val
	gameplay_menu._on_difficulty_value_changed(test_val) 
	
	assert_eq(_resource.difficulty, test_val, "Resource should update from slider input [cite: 208]")
	assert_eq(gameplay_menu.difficulty_label.text, "{" + str(test_val) + "}", "Label should reflect new value [cite: 208]")


## GS-UI-04/05 | Reset button returns to default state
func test_gs_ui_04_05_reset_functionality() -> void:
	# Pre-condition: Set to non-default [cite: 203]
	gameplay_menu.difficulty_slider.value = 2.0
	gameplay_menu._on_gameplay_reset_button_pressed()
	
	assert_eq(_resource.difficulty, 1.0, "Resource should reset to 1.0 [cite: 203]")
	assert_eq(gameplay_menu.difficulty_slider.value, 1.0, "Slider UI should reset to 1.0 [cite: 203]")
...
func test_gs_ui_06_no_duplicate_propagation() -> void:
	...
	# Verify the resource was only updated once to prevent event loops [cite: 59, 186]
...
func test_gs_obs_01_02_04_external_reactivity() -> void:
	...
	# Simulate external code changing the global resource [cite: 197, 198]
	_resource.setting_changed.emit("difficulty", external_val)
	
	assert_eq(gameplay_menu.difficulty_slider.value, external_val, "Slider must sync with external changes [cite: 199]")
	assert_eq(gameplay_menu.difficulty_label.text, "{" + str(external_val) + "}", "Label must sync with external changes [cite: 199]")
...
func test_gs_obs_03_no_recursion_on_external_update() -> void:
	...
	# Trigger the observer [cite: 197, 198]
	_resource.setting_changed.emit("difficulty", 0.7)
	
	# Verify the slider updated without re-emitting its own value_changed signal [cite: 199]
...
func test_gs_obs_05_filters_unrelated_settings() -> void:
	...
	# Emit a signal for a different setting [cite: 198]
