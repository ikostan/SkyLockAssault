...
func test_load_volumes_defaults_on_missing() -> void:
	...
	assert_float(manager.get_bus_state(AudioConstants.BUS_MASTER)["volume"]).is_equal(1.0)
	assert_bool(manager.get_bus_state(AudioConstants.BUS_MASTER)["muted"]).is_false()
	assert_float(manager.get_bus_state(AudioConstants.BUS_MUSIC)["volume"]).is_equal(1.0)
	assert_bool(manager.get_bus_state(AudioConstants.BUS_MUSIC)["muted"]).is_false()
	assert_float(manager.get_bus_state(AudioConstants.BUS_SFX)["volume"]).is_equal(1.0)
	assert_bool(manager.get_bus_state(AudioConstants.BUS_SFX)["muted"]).is_false()
	assert_float(manager.get_bus_state(AudioConstants.BUS_SFX_WEAPON)["volume"]).is_equal(1.0)
	assert_bool(manager.get_bus_state(AudioConstants.BUS_SFX_WEAPON)["muted"]).is_false()
	assert_float(manager.get_bus_state(AudioConstants.BUS_SFX_ROTORS)["volume"]).is_equal(1.0)
	assert_bool(manager.get_bus_state(AudioConstants.BUS_SFX_ROTORS)["muted"]).is_false()


## Tests save/load round-trip.
## :rtype: void
func test_save_and_load_volumes() -> void:
	manager.set_bus_state(AudioConstants.BUS_MASTER, 0.5, true)
	manager.save_volumes(test_path)
	assert_bool(FileAccess.file_exists(test_path)).is_true()
	
	manager.set_bus_state(AudioConstants.BUS_MASTER, 1.0, false)  # Reset
	manager.load_volumes(test_path)
	assert_float(manager.get_bus_state(AudioConstants.BUS_MASTER)["volume"]).is_equal(0.5)
	assert_bool(manager.get_bus_state(AudioConstants.BUS_MASTER)["muted"]).is_true()
