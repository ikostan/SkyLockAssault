...
func test_fire_instantiation() -> void:
	...
	assert_float(bullet.linear_velocity.y).is_equal_approx(-400.0, 10.0)  # Updated to match current speed
	weapon.queue_free()
...
func test_firing_bullet() -> void:
	...
	assert_int(bullets.size()).is_equal(1)
	var bullet: = bullets[0]
	assert_float(bullet.linear_velocity.y).is_equal_approx(-400.0, 10.0)  # Updated speed
