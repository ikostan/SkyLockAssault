...
func test_ui_sync_on_ready() -> void:
	...
	# (button_pressed = true means sound is ON). So if muted is true, pressed is false.
...
func test_double_pop_prevented() -> void:
	...
	var mock_prev_2: Control = auto_free(Control.new())
